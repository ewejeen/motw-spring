...
function emailCheck(){
	...
	$.ajax({
		...
		success : function(data){
			if(data.result == 1 || validateEmail(email.val()) == false) {
				email.css({
					'background' : 'url("/resources/img/cancel.png") no-repeat 50%',
					'background-size' : '20px 20px',
					'background-position' : '96% 50%'
				});
				$('#emailCh').val('unchecked');
			} else {
				email.css({
					'background' : 'url("/resources/img/checked.png") no-repeat 50%',
					'background-size' : '20px 20px',
					'background-position' : '96% 50%'
				});
				$('#emailCh').val('checked');
